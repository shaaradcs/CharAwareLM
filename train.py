...
for epoch in range(0, 30):
    ...
    for step in tqdm(range(0, len(data)//batch_size)):
        ...
        for i in range(0, batch_size):
            train_loss += criterion(results[i], word_index[i][1:-1])
        train_loss.backward()
    ...
    with torch.no_grad():
        if torch.cuda.is_available():
            ...
            validation_loss = torch.zeros(1).float()
        for line in tqdm(valid_data):
            ...
            loss = criterion(result[0], word_index[1:-1])
            """
    ...
    pickle.dump(model, open('models/model_82/epoch_' + str(epoch + 1) + '.pkl','wb'))
